    ...
    async execute(interaction, client) {
        ...
        if (interaction.isModalSubmit()) {
            ...
            try {
                ...
                if (error.code === 'MODULE_NOT_FOUND' && error.requireStack && error.requireStack[0].includes(`handlers/${action}`)) {
                    console.warn('Handler manquant pour modal:', interaction.customId);
                    await interaction.reply({ content: '❌ Action non reconnue.', flags: 64 });
                } else {
